...
{
    internal class CustomButtonDrawable : IDrawable
    {
        ...
        canvas.FillCircle(centerOfCircle, limitingDim / 2);
            ...
            using (Stream stream = assembly.GetManifestResourceStream("MovieRatingApp.Images." + Image))
            {
                image = Microsoft.Maui.Graphics.Platform.PlatformImage.FromStream(stream);
                if (image is null)
                    throw new FileNotFoundException("MovieRatingApp.Images." + Image);
                if (image != null)
                {
